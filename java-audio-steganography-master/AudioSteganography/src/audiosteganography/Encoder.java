	...
	public void encodeMessage(String message, String outPath) { //change outPath to File
		...
		int currentBit = 0;
        	try {
        		...
        		int bytesRead = 0;
	    		int nbChannels = sampleReader.getFormat().getChannels(); 		//no. of channels
	    		int totalBytes = (int) sampleReader.getSampleCount()*nbChannels;    //no. of samples * no. of channels
	    		...
	    		sampleReader.getInterleavedSamples(0, totalBytes, audioData);  //audioData contains each sample of audio converted to int and divided by a ratio
			...
			//writing out the leftover part of the audio file (which doesn't have any encoded bits in it)
